import { Box, TextField, Button, Typography, Paper, Stack, InputAdornment, IconButton, Card, Switch, FormControlLabel, Divider, useTheme, alpha, CircularProgress } from "@mui/material";
import { useState } from "react";
import { useFolders } from "../hooks/useFolders";
import { CreateNewFolder, Lock, Visibility, VisibilityOff, Person } from "@mui/icons-material";
...
export default function Create() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addFolder({ name, createdBy, password: isPrivate ? password : undefined });
  ...
  return (
    <Box>
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" sx={{ fontWeight: 700, mb: 1 }}>Create New Folder</Typography>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          ...
          <Box>
            <Typography variant="h6" sx={{ fontWeight: 600 }}>Folder Details</Typography>
      ...
      <FeedbackModal
        ...
        title={feedbackType === 'success' ? 'Success!' : feedbackType === 'error' ? 'Error' : 'Creating Folder'}
        message={feedbackMessage}
        type={feedbackType}
        actionText={feedbackType === 'success' ? 'Go to Folders' : undefined}
        onAction={feedbackType === 'success' ? () => navigate('/folders') : undefined}
      />
    </Box>
  );
}