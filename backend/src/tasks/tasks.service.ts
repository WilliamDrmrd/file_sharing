...
export class TasksService implements OnModuleInit {
      ...
      media.map(async (file) => {
        try {
          if (!file.originalFilename) {
            this.logger.warn(
              `File ${file.id} does not have an original filename.`,
            );
            return;
          }
          ...
          const thumbnailUrl = file.thumbnailUrl
            ? await this.generateSignedUrlRead(
                process.env.GCLOUD_BUCKET_NAME_THUMBNAIL ||
                  'default-bucket-name',
          ...
          this.logger.error(
            `Failed generate signed url for file ${file.id}: ${error.message}`,
          );
          return null;
        }
      }),
    ...
    await Promise.all(
      folders.map(async (folder) => {
        try {
          ...
          this.logger.error(
            `Failed to generate signed url for folder ${folder.id}: ${error.message}`,
          );
          return null;
        }
      }),
  ...
  private async updateSignedUrls() {
    try {
      ...
      this.logger.log(
        '--Successfully updated signed URLs for all media files.',
      );
